...
for color, colorName in colors:
    dst = cv.warpAffine(img, M, (rows, cols), flags=cv.INTER_NEAREST, borderMode=cv.BORDER_CONSTANT, borderValue=color)
    writeImg(f'test_border_value_{colorName}.png',dst)
