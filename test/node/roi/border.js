...
describe('we check that each ROI is surrounded by the expect zones', function () {
    it('should yield the right contours size', function () {
        return load('BW11x11.png').then(function (img) {
            ...
            rois[1].should.containEql({surround: [1], surface:72, external: 32, contour: 32, border: 44});
            rois[2].should.containEql({surround: [-2], surface:39, external: 39, contour: 39, border: 39});
            rois[3].should.containEql({surround: [-1], surface:9, external: 8, contour: 8, border: 8});
        });
    });
});
