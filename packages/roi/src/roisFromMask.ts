...
export function roisFromMask(
  mask: Image,
  options: IRoisFromMaskOptions = {}
): RoiList {
  const { allowCorners } = options;

  const MAX_ARRAY = 0x00ffff; // 65535 should be enough for most of the cases

  // based on a binary image we will create plenty of small images
  const data = new Int16Array(mask.size); // maxValue: 32767, minValue: -32768

  // split will always return an array of images
  let positiveID = 0;
  let negativeID = 0;

  const xToProcess = new Uint16Array(MAX_ARRAY + 1); // assign dynamically ????
  const yToProcess = new Uint16Array(MAX_ARRAY + 1); // mask +1 is of course mandatory !!!
  ...
  function analyseSurface(x: number, y: number): void {
    let from = 0;
    let to = 0;
    const targetState = mask.getValue(y, x, 0);
    const id = targetState !== 0 ? ++positiveID : --negativeID;
    ...
    while (from <= to) {
      ...
      if (
        ...
        mask.getValue(currentY, currentX - 1, 0) === targetState
      ) {
        // LEFT
        to++;
      ...
      if (
        ...
        mask.getValue(currentY - 1, currentX, 0) === targetState
      ) {
        // TOP
        to++;
      ...
      if (allowCorners) {
        if (
          ...
          mask.getValue(currentY - 1, currentX - 1, 0) === targetState
        ) {
          // TOP LEFT
          to++;
          xToProcess[to & MAX_ARRAY] = currentX - 1;
  ...
  return new RoiList(data, mask.width, mask.height);
}
