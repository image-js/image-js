...
describe('check the setBorder transform', function () {
  it('check the pad for GREY image', function () {
    ...
    expect(
      Array.from(
        image.setBorder({ size: 1, algorithm: 'set', color: [20] }).data,
      ),
    ).toStrictEqual([
      20, 20, 20, 20, 20, 6, 7, 20, 20, 10, 11, 20, 20, 20, 20, 20,
    ]);
  });
  ...
  it('check the pad for larger GREY image', function () {
    let image = new Image(
      ...
      6,
      [
        1, 1, 1, 2, 2, 2, 1, 1, 1, 2, 2, 2, 1, 1, 1, 2, 2, 2, 3, 3, 3, 4, 4, 4,
    ...
    expect(
      Array.from(
        image.setBorder({ size: 2, algorithm: 'set', color: [10] }).data,
      ),
    ).toStrictEqual([
      10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 1, 2, 10, 10, 10,
      10, 3, 4, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10,
    ]);
