...
describe('Grey transform', function () {
  it('RGBA image', function () {
    ...
    expect(Array.from(image.grey({ algorithm: 'min' }).data)).toStrictEqual([
      100,
      0,
    ]);
    expect(
      Array.from(image.grey({ algorithm: 'minimum' }).data),
    ).toStrictEqual([100, 0]);
    expect(Array.from(image.grey({ algorithm: 'max' }).data)).toStrictEqual([
      200,
      0,
    ]);
    expect(
      Array.from(image.grey({ algorithm: 'maximum' }).data),
    ).toStrictEqual([200, 0]);
    expect(
      Array.from(image.grey({ algorithm: 'brightness' }).data),
    ).toStrictEqual([200, 0]);
    expect(Array.from(image.grey({ algorithm: 'red' }).data)).toStrictEqual([
      100,
      0,
    ]);
    ...
    expect(
      Array.from(image.grey({ algorithm: 'magenta' }).data),
    ).toStrictEqual([63, 0]);
    ...
    expect(Array.from(image.grey({ algorithm: 'hue' }).data)).toStrictEqual([
      148,
      0,
    ]);
    expect(
      Array.from(image.grey({ algorithm: 'saturation' }).data),
    ).toStrictEqual([128, 0]);
    expect(
      Array.from(image.grey({ algorithm: 'lightness' }).data),
    ).toStrictEqual([150, 0]);
    expect(
      Array.from(image.grey({ algorithm: 'luminosity' }).data),
    ).toStrictEqual([150, 0]);
    expect(
      Array.from(image.grey({ algorithm: 'luminance' }).data),
    ).toStrictEqual([150, 0]);
    ...
    expect(Array.from(image.grey({ mergeAlpha: true }).data)).toStrictEqual([
      142,
      0,
    ]);
