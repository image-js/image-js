...
describe('we check the validateArrayOfChannels method', function () {
  it('check for a RGB image', function () {
    ...
    expect(validateArrayOfChannels(image, { channels: 'r' })).toStrictEqual([
      0,
    ]);
    expect(validateArrayOfChannels(image, { channels: 'b' })).toStrictEqual([
      2,
    ]);
    expect(validateArrayOfChannels(image, { channels: 1 })).toStrictEqual([1]);
    expect(
      validateArrayOfChannels(image, { channels: [0, 1, 2] }),
    ).toStrictEqual([0, 1, 2]);
  ...
  it('check for a RGBA image', function () {
    ...
    expect(
      validateArrayOfChannels(image, { channels: ['r', 'g', 'b'] }),
    ).toStrictEqual([0, 1, 2]);
    expect(validateArrayOfChannels(image, { channels: 'r' })).toStrictEqual([
      0,
    ]);
    ...
    expect(
      validateArrayOfChannels(image, { defaultAlpha: false }),
    ).toStrictEqual([0, 1, 2]);
    expect(
      validateArrayOfChannels(image, { allowAlpha: true, defaultAlpha: false }),
    ).toStrictEqual([0, 1, 2]);
    expect(
      validateArrayOfChannels(image, { allowAlpha: true, defaultAlpha: true }),
    ).toStrictEqual([0, 1, 2, 3]);
    expect(validateArrayOfChannels(image, { channels: 'a' })).toStrictEqual([
      3,
    ]);
    expect(
      validateArrayOfChannels(image, { channels: ['r', 'a'] }),
